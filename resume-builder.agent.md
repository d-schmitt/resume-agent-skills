...
After profile extraction, review the **Missing Info Questions** generated in Step 3b of the cv-1-profile-extraction skill.
...
Use the `cv-2-job-analysis` skill to analyze the target role.
- Read the skill instructions from `skills/cv-2-job-analysis/SKILL.md`
...
- Record the confirmed output language and pass it to the cv-3-tailoring skill.

### Step 4: Tailor the CV
Use the `cv-3-tailoring` skill to produce the CV content.
