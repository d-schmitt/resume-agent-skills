...
1. **[cv-profile-extraction](skills/cv-profile-extraction/SKILL.md)** — parses your LinkedIn PDF, existing CV, LinkedIn URL, or pasted text into structured profile data
2. **[cv-job-analysis](skills/cv-job-analysis/SKILL.md)** — extracts requirements, ATS keywords, and priority ratings from any job description
3. **[cv-tailoring](skills/cv-tailoring/SKILL.md)** — performs gap analysis, asks targeted questions to surface achievements, writes the full CV, and scores it (0–100) on keyword coverage, role fit, and content quality
4. **[cv-export](skills/cv-export/SKILL.md)** — converts CV content to DOCX and PDF; the agent detects its environment and installs any required packages automatically
...
4. Under **Knowledge**, upload all skill files so the agent can read them during the workflow:
   - `skills/cv-profile-extraction/SKILL.md`
   - `skills/cv-job-analysis/SKILL.md`
...
Create separate agents for each skill (cv-profile-extraction, cv-job-analysis, cv-tailoring, cv-export) with their respective `SKILL.md` as instructions. Then create an orchestrator agent using `resume-builder.agent.md` and attach the skill agents via **Actions → Other Agents** for delegation.
